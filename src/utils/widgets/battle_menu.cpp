...
void physicalAttack(Player &player, Monster &monster)
{
    ...
    text.toCentered(player.getName() + " dealt a total of " + text.toString(totalDmg, 2) +" damage!");

    if(!monster.isAlive()){
        text.toCentered(monster.getName() + " couldn't withstand the damage!\n", 160, {}, true);
    }
    else{
        text.toCentered(monster.getName() + " resisted " + text.toString(percentage, 2) + "%!");
    }
}
...
void takeDamage(Player &player, Monster &monster)
{
    ...
    text.toCentered(monster.getName() + " attacks with " + text.toString(monster.getDamage(), 2) + " damage!");

    if(!player.isAlive()){
        text.toCentered(player.getName() + " couldn't withstand the damage!\n", 160, {}, true);
    }
    else{
        text.toCentered(player.name + " resisted " + text.toString(percentage, 2) + "%!");
    }
}
//  Pokemon battle ahh.
    ...
    do{
        ...
        switch(choice){
            case 'a': //  Player attacks monster.
                ...
                sleep(1500);
                text.toCentered(monster.getName() + " has " + text.toString(monster.getCurrentHealth(), 2) + " health left.");
                break;
        ...
        text.toCentered(player.getName() + " has " + text.toString(player.getCurrentHealth(), 2) + " health left.\n");
        sleep(2000);
